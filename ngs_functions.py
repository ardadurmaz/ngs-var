...
def ngs_index(inputs):
    if re.search("bwa", str.lower(inputs.indexer)):
        if len(glob.glob(os.path.abspath(inputs.reference + '.bwt'))) > 0:
            ...
            if inputs.verbose:
                print("Running: %s index %s" % (inputs.bwa, inputs.reference))
            if not inputs.dry:
        ...
        if re.search('fastp', str.lower(inputs.trimmer)):
            ...
            for r in targets_data:
                if os.path.exists(
                    ...
                    if inputs.verbose:
                        print("Running: %s -q 15,10 -A XXX -m 30 -o %s -p %s %s %s" % (
                            inputs.cutadapt,
                            os.path.abspath(
                                inputs.dir + '/readsTrimmed/Trimmed_Read1_' + r._id + '_' + r._lib + '_' + r._lane + '.fastq'),
                            os.path.abspath(
            ...
            for r in targets_data:
                if os.path.exists(
                    ...
                    if inputs.verbose:
                        print("Running: %s -m pe -q 10 -Q 20 -l 35 -n -t %d -o %s %s %s" % (
                            inputs.skewer,
                            inputs.threads,
                            os.path.abspath(
    ...
    if re.search('bwa', str.lower(inputs.aligner)):
        for r in targets_data:
            if os.path.exists(
                ...
                if inputs.verbose:
                    print("Running: %s mem -t %d -M -R \'@RG\\tID:%s\\tSM:%s\\tPL:%s\\tLB:%s\' %s %s %s > %s" % (
                        inputs.bwa,
                        inputs.threads,
...
def ngs_mark_sort(inputs, targets_data):
    for r in targets_data:
        if os.path.exists(
            ...
            if inputs.verbose:
                print("Running: %s view -bh -@ %d -o %s %s" % (
                    inputs.samtools,
                    inputs.threads,
            ...
            if not inputs.dry:
                os.system("%s view -bh -@ %d -o %s %s" % (
                    ...
                    r._aligned))
            if inputs.verbose:
                print("Running: %s markdup -r -p -t %d %s %s" % (
                    inputs.sambamba,
                    inputs.threads,
                    os.path.abspath(inputs.dir + '/bamProcessed/' + r._id + '_' + r._lib + '_' + r._lane + '.bam'),
    ...
    if str.upper(inputs.workflow_type) == 'SOMATICSNVINDEL':
        ...
        for s in sample_ids:
            if (os.path.exists(
                ...
                if inputs.verbose:
                    print("Running java -jar %s MergeSamFiles I=%s O=%s" % (
                        inputs.gatk,
            ...
            if (os.path.exists(
                ...
                if inputs.verbose:
                    if not (inputs.bed is None):
                        print("Running java -jar %s -T BaseRecalibrator -nct %d -R %s -I %s -knownSites %s -knownSites %s -o %s -L %s" % (
                        ...
                        print("Running java -jar %s -T BaseRecalibrator -nct %d -R %s -I %s -knownSites %s -knownSites %s -o %s" % (
                            inputs.gatk, inputs.threads,
                            inputs.reference,
                            s._bamTumor,
                            inputs.dbSNP,
    ...
    for s in targets_data_processed:
        if str.upper(inputs.workflow_type) == "SOMATICSNVINDEL":
            if (os.path.exists(
                ...
                if inputs.verbose:
                    print("Running: java -jar %s -T PrintReads -nct %d -R %s -I %s -BQSR %s -o %s" % (
                        inputs.gatk, inputs.threads,
                        inputs.reference,
                        s._bamTumor,
...
def ngs_haplotypecaller(inputs, targets_data_processed):
    ...
    if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/GATK_HCaller_' + inputs.title + '.raw.snps.indels.vcf')):
        ...
        if inputs.verbose:
            print("Running: java -jar %s -R %s -I %s --num_cpu_threads_per_data_thread %d -stand_call_conf 20 -o %s "\
