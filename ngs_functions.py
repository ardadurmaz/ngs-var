        ...
        if isinstance(val, str):
            if not (str.upper(val) == "GERMLINESNVINDEL" or
                ...
                if not (os.path.exists(os.path.abspath(val))):
                    print_log(inputs, "  Check Argument: [%s --> %s]" % (arg, val))
                    _FLAG = False
...
def read_config(args):
    ...
    if len(local_data.keys()) == 0:
        print_log(inputs, "!! Error in reading the config file")
        sys.exit(1)
    ...
    if args.trimmer is not None:
        ...
        elif re.search('fastp', str.lower(args.trimmer)):
            inputs.trimmer = "FASTP"
    else:
        inputs.trimmer = None
...
def get_inputs():
    ...
    subparsers = parser.add_subparsers(description='Workflow to process', dest='workflow_type')

    ## Germline ##
    parser_germline = subparsers.add_parser('GermlineSNVIndel', help='Germline SNV & Indel Detection')
    ...
    parser_germline.add_argument('--dry', action="store_true", help='Dry-Run')
    parser_germline.add_argument('--clear', action='store_true', default=False, help='Clear workspace')
    parser_germline.add_argument('--threads', default=2, type=int, help="Number of threads to use")
    parser_germline.add_argument('--title', default='NGS_Analysis', help='Title of the run')
    parser_germline.add_argument('--ped', default=None, help='Ped file for family analysis')
    parser_germline.add_argument('--bed', default=None, help='Bed file for regions')
    parser_germline.add_argument('--exome', action='store_true', default=False, help='Whole Exome Data')

    ## Somatic ##
    parser_somatic = subparsers.add_parser('SomaticSNVIndel', help='Somatic SNV & Indel Detection')
    ...
    parser_somatic.add_argument('--dry', action="store_true", help='Dry-Run')
    parser_somatic.add_argument('--clear', action='store_true', default=False, help='Clear workspace')
    parser_somatic.add_argument('--threads', default=2, type=int, help='Number of threads to use')
    parser_somatic.add_argument('--title', default='Sample_Analysis', help='Title of the run')
    parser_somatic.add_argument('--bed', default=None, help='Bed file for regions')
    parser_somatic.add_argument('--exome', action='store_true', default=False, help='Whole Exome Data')
    ...
    parser_expr.add_argument('--verbose', action="store_true", help='Verbosity')
    parser_expr.add_argument('--dry', action="store_true", help='Dry-Run')
    parser_expr.add_argument('--clear', action='store_true', default=False, help='Clear workspace')
    parser_expr.add_argument('--threads', default=2, type=int, help='Number of threads to use')
    parser_expr.add_argument('--title', default='Sample_Analysis', help='Title of the run')
        ...
        for l in fh:
            ...
            if lCount == 1:
                ...
                if len(list(filter(lambda x: re.search('^[^\s]+$', x), local))) != len(indx):
                    print_log(inputs, '!Warning, format issue. Line %d skipping' % lCount)
                    next(fh)
                else:
...
def prep_wd(path_wd, workflow_type, inputs):
    ...
    if str.upper(workflow_type) == "GERMLINESNVINDEL" or str.upper(workflow_type) == "SOMATICSNVINDEL":
        for _dir in ["readsTrimmed", "readsAligned", "bamProcessed", "bamMerged", "bamCalib", "metrics", "indices",
                     "recalib", "rawSNP", "filtSNP"]:
            local_path = os.path.abspath(path_wd + '/' + _dir)
            if not os.path.exists(local_path):
                try:
                    if inputs.verbose:
                        print_log(inputs, "Creating Directory: %s" % local_path)
                    if not inputs.dry:
                        os.mkdir(local_path)
                except:
                    _flag = False
                ...
                if inputs.clear:
                    if inputs.verbose:
                        print_log(inputs, "Deleting Directory: %s" % local_path)
                    if not inputs.dry:
...
def ngs_index(inputs):
    if re.search("bwa", str.lower(inputs.indexer)):
        if len(glob.glob(os.path.abspath(inputs.reference + '.bwt'))) > 0:
            print_log(inputs, "    *** indices found, skipping..")
        else:
            if inputs.verbose:
                print_log(inputs, "Running: %s index %s" % (inputs.bwa, inputs.reference))
            if not inputs.dry:
                os.system("%s index %s" % (inputs.bwa, inputs.reference))
        ...
        if re.search('fastp', str.lower(inputs.trimmer)):
            for r in targets_data:
                ...
                if(os.path.exists(trimmedR1_Name) and os.path.exists(trimmedR2_Name)):
                    ...
                    if inputs.verbose:
                        print_log(inputs, "[INFO:COMMAND] %s --thread 12 --compression 9 --trim_front1 10 --trim_front2 10 --cut_tail --detect_adapter_for_pe --in1 %s --in2 %s --out1 %s --out2 %s" % (inputs.fastp,r._r1,r._r2, trimmedR1_Name,trimmedR2_Name))
                    if not inputs.dry :
            ...
            for r in targets_data:
                if os.path.exists(
                        os.path.abspath(inputs.dir + '/readsTrimmed/Trimmed_Read1_' + r._id + '_' + r._lib + '_' + r._lane + '.fastq')) and \
                        os.path.exists(
                    ...
                    r._trimmedR1 = os.path.abspath(
                        inputs.dir + '/readsTrimmed/Trimmed_Read1_' + r._id + '_' + r._lib + '_' + r._lane + '.fastq')
                    r._trimmedR2 = os.path.abspath(
                        inputs.dir + '/readsTrimmed/Trimmed_Read2_' + r._id + '_' + r._lib + '_' + r._lane + '.fastq')
                else:
                    if inputs.verbose:
                        print_log(inputs, "Running: %s -q 15,10 -A XXX -m 30 -o %s -p %s %s %s" % (
                            inputs.cutadapt,
                            os.path.abspath(
                                inputs.dir + '/readsTrimmed/Trimmed_Read1_' + r._id + '_' + r._lib + '_' + r._lane + '.fastq'),
                            os.path.abspath(
                    ...
                    if not inputs.dry:
                        os.system(
                            "%s -q 15,10 -A XXX -m 30 -o %s -p %s %s %s" % (
                                inputs.cutadapt,
                                os.path.abspath(
                                    inputs.dir + '/readsTrimmed/Trimmed_Read1_' + r._id + '_' + r._lib + '_' + r._lane + '.fastq'),
                                os.path.abspath(
            ...
            for r in targets_data:
                ...
                if os.path.exists(
                        os.path.abspath(inputs.dir + '/readsTrimmed/' + stagr1 + '.good.fq.gz')) and \
                        os.path.exists(
                                os.path.abspath(inputs.dir + '/readsTrimmed/' + stagr2 + '.good.fq.gz')):
                    print_log(inputs, "    *** trimmed reads found, skipping...")
                    ...
                    if inputs.verbose:
                        print_log(inputs, 
                            "Running: python %s -1 %s -2 %s --debubble False --gzip --compression 9 -g %s --no_correction" % (
                                inputs.afterqc,
                                r._r1, r._r2,
                                os.path.abspath(inputs.dir + '/readsTrimmed/')))
                    if not inputs.dry:
                        os.system(
                            "python %s -1 %s -2 %s --debubble False --gzip --compression 9 -g %s --no_correction" % (
                                inputs.afterqc,
                                r._r1, r._r2,
                                os.path.abspath(inputs.dir + '/readsTrimmed/')))
                    r._trimmedR1 = os.path.abspath(inputs.dir + '/readsTrimmed/' + stagr1 + '.good.fq.gz')
            ...
            for r in targets_data:
                if os.path.exists(
                        os.path.abspath(
                            inputs.dir + '/readsTrimmed/' + r._id + '_' + r._lib + '_' + r._lane + '-trimmed-pair1.fastq')) and \
                        os.path.exists(
                                os.path.abspath(
                                    inputs.dir + '/readsTrimmed/' + r._id + '_' + r._lib + '_' + r._lane + '-trimmed-pair2.fastq')):
                    print_log(inputs, "    *** trimmed reads found, skipping...")
                    r._trimmedR1 = os.path.abspath(
                        inputs.dir + '/readsTrimmed/' + r._id + '_' + r._lib + '_' + r._lane + '-trimmed-pair1.fastq')
                    r._trimmedR2 = os.path.abspath(
                        inputs.dir + '/readsTrimmed/' + r._id + '_' + r._lib + '_' + r._lane + '-trimmed-pair2.fastq')
                else:
                    if inputs.verbose:
                        print_log(inputs, "Running: %s -m pe -q 10 -Q 20 -l 35 -n -t %d -o %s %s %s" % (
                            inputs.skewer,
                            inputs.threads,
                            os.path.abspath(
                                inputs.dir + '/readsTrimmed/' + r._id + '_' + r._lib + '_' + r._lane),
                            r._r1, r._r2))
                    if not inputs.dry:
                        os.system("%s -m pe -q 10 -Q 20 -l 35 -n -t %d -o %s %s %s" % (
                            inputs.skewer,
                            inputs.threads,
                            os.path.abspath(
                                inputs.dir + '/readsTrimmed/' + r._id + '_' + r._lib + '_' + r._lane),
...
def ngs_align(inputs, targets_data):
    if re.search('bwa', str.lower(inputs.aligner)):
        for r in targets_data:
            if os.path.exists(
                    os.path.abspath(
                        inputs.dir + '/readsAligned/Aligned_' + r._id + '_' + r._lib + '_' + r._lane + '_bwa.sam')):
                print_log(inputs, "    *** aligned reads found, skipping...")
                r._aligned = os.path.abspath(
                    inputs.dir + '/readsAligned/Aligned_' + r._id + '_' + r._lib + '_' + r._lane + '_bwa.sam')
            else:
                if inputs.verbose:
                    print_log(inputs, "Running: %s mem -t %d -M -R \'@RG\\tID:%s\\tSM:%s\\tPL:%s\\tLB:%s\' %s %s %s > %s" % (
                        inputs.bwa,
                        inputs.threads,
                ...
                if not inputs.dry:
                    os.system("%s mem -t %d -M -R \'@RG\\tID:%s\\tSM:%s\\tPL:%s\\tLB:%s\' %s %s %s > %s" % (
                        inputs.bwa,
                        inputs.threads,
                        r._id + '_' + r._lib + '_' + r._lane,
                        r._id,
...
def ngs_mark_sort(inputs, targets_data):
    for r in targets_data:
        if os.path.exists(
                os.path.abspath(
                    inputs.dir + '/bamProcessed/Processed_' + r._id + '_' + r._lib + '_' + r._lane + '.bam')):
            print_log(inputs, "    *** marked files found, skipping...")
            r._processed = os.path.abspath(
                inputs.dir + '/bamProcessed/Processed_' + r._id + '_' + r._lib + '_' + r._lane + '.bam')
        else:
            if inputs.verbose:
                print_log(inputs, "Running: %s view -bh -@ %d -o %s %s" % (
                    inputs.samtools,
                    inputs.threads,
                    os.path.abspath(inputs.dir + '/bamProcessed/' + r._id + '_' + r._lib + '_' + r._lane + '.bam'),
                    r._aligned))
            if not inputs.dry:
                os.system("%s view -bh -@ %d -o %s %s" % (
                    inputs.samtools,
                    inputs.threads,
                    os.path.abspath(inputs.dir + '/bamProcessed/' + r._id + '_' + r._lib + '_' + r._lane + '.bam'),
                    r._aligned))
            if inputs.verbose:
                print_log(inputs, "Running: %s markdup -r -p -t %d %s %s" % (
                    inputs.sambamba,
                    inputs.threads,
                    os.path.abspath(inputs.dir + '/bamProcessed/' + r._id + '_' + r._lib + '_' + r._lane + '.bam'),
                    os.path.abspath(inputs.dir + '/bamProcessed/Rmdup_' + r._id + '_' + r._lib + '_' + r._lane + '.bam')))
            if not inputs.dry:
                os.system("%s markdup -r -p -t %d %s %s" % (
                    inputs.sambamba,
                    inputs.threads,
                    os.path.abspath(inputs.dir + '/bamProcessed/' + r._id + '_' + r._lib + '_' + r._lane + '.bam'),
                    os.path.abspath(inputs.dir + '/bamProcessed/Rmdup_' + r._id + '_' + r._lib + '_' + r._lane + '.bam')))
            if inputs.verbose:
                print_log(inputs, "Running: %s sort -@ %d -o %s %s" % (
                    inputs.samtools,
                    inputs.threads,
                    os.path.abspath(inputs.dir + '/bamProcessed/Processed_' + r._id + '_' + r._lib + '_' + r._lane + '.bam'),
                    os.path.abspath(inputs.dir + '/bamProcessed/Rmdup_' + r._id + '_' + r._lib + '_' + r._lane + '.bam')))
            if not inputs.dry:
                os.system("%s sort -@ %d -o %s %s" % (
                    inputs.samtools,
                    inputs.threads,
                    os.path.abspath(inputs.dir + '/bamProcessed/Processed_' + r._id + '_' + r._lib + '_' + r._lane + '.bam'),
                    os.path.abspath(inputs.dir + '/bamProcessed/Rmdup_' + r._id + '_' + r._lib + '_' + r._lane + '.bam')))
...
def ngs_merge(inputs, targets_data, targets_data_processed):
    ...
    if str.upper(inputs.workflow_type) == 'SOMATICSNVINDEL':
        ...
        for s in sample_ids:
            if (os.path.exists(
                    os.path.abspath(inputs.dir + '/bamMerged/Merged_Tumor_' + s + '.bam')) and
                    os.path.exists(
                        os.path.abspath(inputs.dir + '/bamMerged/Merged_Normal_' + s + '.bam'))):
                print_log(inputs, "    *** Merged bam files found, skipping...")
                targets_data_processed.append(
                    ngs_classes.WxsSample(_id=s,
                                          _bamTumor=os.path.abspath(
                                              inputs.dir + '/bamMerged/Merged_Tumor_' + s + '.bam'),
                                          _bamNormal=os.path.abspath(
                                              inputs.dir + '/bamMerged/Merged_Normal_' + s + '.bam')))
            else:
                if inputs.verbose:
                    print_log(inputs, "Running java -jar %s MergeSamFiles I=%s O=%s" % (
                        inputs.gatk,
                        ' I='.join(sample_ids[s]["TUMOR"]),
                        os.path.abspath(
                            inputs.dir + '/bamMerged/Merged_Tumor_' + s + '.bam')))
                if not inputs.dry:
                    os.system("java -jar %s MergeSamFiles I=%s O=%s" % (
                        inputs.gatk,
                        ' I='.join(sample_ids[s]["TUMOR"]),
                        os.path.abspath(
                            inputs.dir + '/bamMerged/Merged_Tumor_' + s + '.bam')))
        ...
        for s in sample_ids:
            if (os.path.exists(
                ...
                targets_data_processed.append(
                    ngs_classes.WxsSample(_id=s,
                                          _bam=os.path.abspath(
                                              inputs.dir + '/bamMerged/Merged_' + s + '.bam')))
...
def bqsr(inputs, targets_data_processed):
    ...
    for s in targets_data_processed:
        if str.upper(inputs.workflow_type) == "SOMATICSNVINDEL":
            if inputs.verbose:
                print_log(inputs, "Running %s index %s" % (inputs.samtools, s._bamTumor))

            if not inputs.dry:
                os.system("%s index %s" % (inputs.samtools, s._bamTumor))

            if inputs.verbose:
                print_log(inputs, "Running %s index %s" % (inputs.samtools, s._bamNormal))
            ...
            if inputs.verbose:
                print_log(inputs, "Running %s index %s" % (inputs.samtools, s._bam))
    ...
    for s in targets_data_processed:
        if str.upper(inputs.workflow_type) == "SOMATICSNVINDEL":
            if (os.path.exists(
                    os.path.abspath(inputs.dir + 'recalib/RecalibrationTableTumor_' + s._id + '.table')) and
                    os.path.exists(
                        os.path.abspath(inputs.dir + 'recalib/RecalibrationTableNormal_' + s._id + '.table'))):
                print_log(inputs, "    *** Recalibration tables found, skipping...")
                s._recalibTableTumor = os.path.abspath(
                    inputs.dir + 'recalib/RecalibrationTableTumor_' + s._id + '.table')
                s._recalibTableNormal = os.path.abspath(
                    inputs.dir + 'recalib/RecalibrationTableNormal_' + s._id + '.table')


            else:
                if inputs.verbose:
                    if not (inputs.bed is None):
                        print_log(inputs, "Running java -jar %s -T BaseRecalibrator -nct %d -R %s -I %s -knownSites %s -knownSites %s -o %s -L %s" % (
                            inputs.gatk, inputs.threads,
                            inputs.reference,
                            s._bamTumor,
                            inputs.dbSNP,
                            inputs.mills,
                        ...
                        print_log(inputs, "Running java -jar %s -T BaseRecalibrator -nct %d -R %s -I %s -knownSites %s -knownSites %s -o %s" % (
                            inputs.gatk, inputs.threads,
                            inputs.reference,
                            s._bamTumor,
                            inputs.dbSNP,
                ...
                if not inputs.dry:
                    if not (inputs.bed is None):
                        os.system("java -jar %s -T BaseRecalibrator -nct %d -R %s -I %s -knownSites %s -knownSites %s -o %s -L %s" % (
                            inputs.gatk, inputs.threads,
                            inputs.reference,
                            s._bamTumor,
                            inputs.dbSNP,
                        ...
                        os.system("java -jar %s -T BaseRecalibrator -nct %d -R %s -I %s -knownSites %s -knownSites %s -o %s" % (
                            inputs.gatk, inputs.threads,
                            inputs.reference,
                            s._bamTumor,
                            inputs.dbSNP,
            ...
            if (os.path.exists(
                ...
                print_log(inputs, "    *** Recalibration table found, skipping...")
                s._recalibTable = os.path.abspath(
                    inputs.dir + 'recalib/RecalibrationTable_' + s._id + '.table')
    ...
    for s in targets_data_processed:
        if str.upper(inputs.workflow_type) == "SOMATICSNVINDEL":
            if (os.path.exists(
                    os.path.abspath(inputs.dir + 'bamCalib/CalibratedTumor_' + s._id + '.bam')) and
                    os.path.exists(
                        os.path.abspath(inputs.dir + 'bamCalib/CalibratedNormal_' + s._id + '.bam'))):
                print_log(inputs, "    *** BQSR adjusted files found, skipping...")
                s._bamCalibTumor = os.path.abspath(
                    inputs.dir + 'bamCalib/CalibratedTumor_' + s._id + '.bam')
                s._bamCalibNormal = os.path.abspath(
                    inputs.dir + 'bamCalib/CalibratedNormal_' + s._id + '.bam')
            else:
                if inputs.verbose:
                    print_log(inputs, "Running: java -jar %s -T PrintReads -nct %d -R %s -I %s -BQSR %s -o %s" % (
                        inputs.gatk, inputs.threads,
                        inputs.reference,
                        s._bamTumor,
                ...
                if not inputs.dry:
                    os.system("java -jar %s -T PrintReads -nct %d -R %s -I %s -BQSR %s -o %s" % (
                        inputs.gatk, inputs.threads,
                        inputs.reference,
                        s._bamTumor,
                        s._recalibTableTumor,
            ...
            if (os.path.exists(
                ...
                print_log(inputs, "    *** BQSR adjusted files found, skipping...")
                s._bamCalib = os.path.abspath(
                    inputs.dir + 'bamCalib/Calibrated_' + s._id + '.bam')
            else:
                if inputs.verbose:
                    print_log(inputs, "Running: java -jar %s -T PrintReads -nct %d -R %s -I %s -BQSR %s -o %s" % (
                        inputs.gatk, inputs.threads,
...
def ngs_mutect(inputs, targets_data_processed):
    for s in targets_data_processed:
        if os.path.exists(os.path.abspath(os.path.abspath(inputs.dir + '/rawSNP/' + r._id + '.vcf'))):
            print_log(inputs, "    *** MuTect raw call file found, skipping...")
        else:
            if inputs.verbose:
                print_log(inputs, "Running %s -jar %s -T MuTect --baq OFF -nct 8 --reference_sequence %s --input_file:normal %s --input_file:tumor %s --out %s --coverage_file %s --vcf %s" % (inputs.java17,
                                                                                                        inputs.mutect1,
                                                                                                        inputs.reference,
                                                                                                        s._bamCalibNormal,
                                                                                                        s._bamCalibTumor,
...
def ngs_haplotypecaller(inputs, targets_data_processed):
    ...
    if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/GATK_HCaller_' + inputs.title + '.raw.snps.indels.vcf')):
        print_log(inputs, "    *** HaplotypeCaller raw file found, skipping...")
    else:
        if inputs.verbose:
            print_log(inputs, "Running: java -jar %s -R %s -I %s --num_cpu_threads_per_data_thread %d -stand_call_conf 20 -o %s "\
        ...
        if not inputs.dry:
            os.system("java -jar %s -R %s -I %s --num_cpu_threads_per_data_thread %d -stand_call_conf 20 -o %s "\
                      "-A BaseQualityRankSumTest -A Coverage -A DepthPerAlleleBySample -A FisherStrand -A QualByDepth -A ReadPosRankSumTest" % (inputs.gatk,
                                                                                                                                                inputs.reference,
                                                                                                                                                " -I ".join(files),
                                                                                                                                                inputs.threads,
    ...
    if not(inputs.ped is None):
        if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/GATK_HCaller_GTPosteriors_' + inputs.title + '.raw.snps.indels.vcf')):
            print_log(inputs, "    *** HaplotypeCaller GenotypePosteriors file found, skipping...")
        else:
            if inputs.verbose:
                print_log(inputs, "Running: java -jar %s -T CalculateGenotypePosteriors -R %s -V %s -o %s --skipPopulationPriors -ped %s" % (inputs.gatk,
                                                                                                                                 inputs.reference,
                                                                                                                                 os.path.abspath(inputs.dir +
                                                                                                                                                 '/rawSNP/GATK_HCaller_' +
                                                                                                                                                 inputs.title + '.raw.snps.indels.vcf'),
...
def ngs_haplotypecaller_filter(inputs):
    ...
    outFile = os.path.abspath(inputs.dir + '/filtSNP/GATK_HCaller_' + inputs.title + '.pre.filtered.snps.indels.vcf')
    
    if os.path.exists(outFile):
        print_log(inputs, "    *** HaplotypeCaller filtered vcf file found, skipping...")
    else:
        if inputs.verbose:
            print_log(inputs, "Running: java -jar %s -T VariantFiltration -R %s -o %s -V %s "\
        ...
        if not inputs.dry:
            os.system("java -jar %s VariantFiltration -R %s -O %s -V %s "\
                      "--filterExpression \'QD < 2.0\' --filterName \"LowQD\" "\
                      "--filterExpression \'FS > 60.0 && vc.getType() == \"SNP\"\' --filterName \"HighFSSNP\" "\
                      "--filterExpression \'FS > 200.0 && vc.getType() == \"INDEL\"\' --filterName \"HighFSINDEL\" "\
                      "--filterExpression \'MQ < 40.0 && vc.getType() == \"SNP\"\' --filterName \"LowMQSNP\" "\
        ...
        if inputs.verbose:
            print_log(inputs, "Running: java -jar %s -T SelectVariants -R %s "\
        ...
        if not inputs.dry:
            os.system("java -jar %s -T SelectVariants -R %s "\
                      "--maxFractionFilteredGenotypes 0.7 --excludeNonVariants --excludeFiltered -selectType SNP -selectType INDEL "\
                      "-o %s -V %s" % (inputs.gatk,
                                       inputs.reference,
                                       os.path.abspath(inputs.dir + '/filtSNP/GATK_HCaller_' + inputs.title + '.filtered.snps.indels.vcf'),
    ...
    if not(inputs.ped is None):
        if os.path.exists(os.path.abspath(inputs.dir + '/filtSNP/GATK_HCaller_' + inputs.title + '.ann.filtered.snps.indels.vcf')):
            print_log(inputs, "    *** Annotated vcf file found, skipping...")
        else:
            if inputs.verbose:
                print_log(inputs, "Running: java -jar %s -T VariantAnnotator -R %s -o %s -V %s -A PossibleDeNovo -ped %s" % (inputs.gatk,
                                                                                                                 inputs.reference,
                                                                                                                 os.path.abspath(inputs.dir +
...
def ngs_bcftools(inputs, targets_data_processed):
    ...
    for s in targets_data_processed:
        if inputs.verbose:
            print_log(inputs, 'Running: %s index %s' % (inputs.samtools, s._bamCalib))
        if not inputs.dry:
    ...
    if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/SAMTools_MPileup_' + inputs.title + '.vcf.gz')):
        print_log(inputs, "    *** Mpileup file found, skipping...")
    else:
        if inputs.verbose:
            print_log(inputs, 'Running: %s mpileup -d 100000 -vsBf %s -t AD,ADF,ADR,DP,SP -o %s %s' % (inputs.samtools,
                                                                                           inputs.reference,
    ...
    if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/BCFTools_Call_' + inputs.title + '.vcf.gz')):
        print_log(inputs, "    *** BCFTools raw call file found, skipping...")
    else:
        if inputs.verbose:
            print_log(inputs, "Running: %s call -mv -O z -o %s --threads %d %s" % (inputs.bcftools,
    ...
    if os.path.exists(os.path.abspath(inputs.dir + '/filtSNP/BCFTools_Filt_' + inputs.title + '.vcf')):
        print_log(inputs, "    *** BCFTools filtered call file found, skipping...")
    else:
        if inputs.verbose:
            print_log(inputs, "Running: %s filter -O v --threads %d -g20 -G10 -i 'FMT/DP>15 & QUAL>20 & GT!~\"\.\" & MQ>40 & RPB>0.1' -o %s %s" % (inputs.bcftools,
                                                                                                                       inputs.threads,
...
def ngs_varscan2_germline(inputs, targets_data_processed):
    ...
    if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/SAMTools_MPileup_' + inputs.title + '.mpileup')):
        ...
        if inputs.verbose:
            print_log(inputs, 'Running: %s mpileup -Bf %s -o %s %s' % (inputs.samtools,
                                                           inputs.reference,
    ...
    if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/VarScan2_Call_' + inputs.title + '.vcf.gz')):
        print_log(inputs, "    *** VarScan2 call file found, skipping...")
    else:
        if inputs.verbose:
            print_log(inputs, "Running: java -jar %s mpileup2cns %s --variants --output-vcf --min-coverage 20 --strand-filter 1 > %s" % (inputs.varscan2,
                                                                                                                             os.path.abspath(inputs.dir + '/rawSNP/SAMTools_MPileup_' + inputs.title + '.mpileup'),
...
def ngs_varscan2_somatic(inputs, targets_data_processed):
    ...
    for s in targets_data_processed:
        if os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/Normal_Tumor_' + s._id + '.mpileup')):
            print_log(inputs, "   *** Pileup file found, skipping...")
            s._pileup = os.path.abspath(inputs.dir + '/rawSNP/Normal_Tumor_' + s._id + '.mpileup')
        else:    
            if inputs.verbose:
                print_log(inputs, "Running: %s mpileup -Bf %s -o %s %s %s" % (inputs.samtools,
                                                                  inputs.reference,
    ...
    for s in targets_data_processed:
        if (os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/' + s._id + '.snp.vcf')) and
           os.path.exists(os.path.abspath(inputs.dir + '/rawSNP/' + s._id + '.indel.vcf'))):
            print_log(inputs, "   *** VarScan2 result files found, skipping...")
            s._snpOut = os.path.abspath(inputs.dir + '/rawSNP/' + s._id + '.snp.vcf')
            s._indelOut = os.path.abspath(inputs.dir + '/rawSNP/' + s._id + '.indel.vcf')
        
        else:
            if inputs.verbose:
                print_log(inputs, "Running: java -jar %s somatic %s %s --mpileup 1 --min-coverage 10 --min-var-freq 0.01 --p-value 0.1 "\
    ...
    for s in targets_data_processed:
        if os.path.exists(os.path.abspath(inputs.dir + '/filtSNP/' + s._id + '.filtered.snp.vcf')):
            print_log(inputs, "    *** VarScan2 filtered result files found, skipping...")
        else:
            if inputs.verbose:
                print_log(inputs, "Running: java -jar %s somaticFilter %s --indel-file %s --output-file %s --p-value 0.1 --min-var-freq 0.01" % (inputs.varscan2,
                                                                                                                                  s._snpOut,
                                                                                                                                  s._indelOut,
                                                                                                                                  os.path.abspath(inputs.dir +
                                                                                                                                                   '/filtSNP/' +
...
def ngs_strelka_somatic(inputs, targets_data_processed):
    ...
    for s in targets_data_processed:
        if inputs.verbose:
            print_log(inputs, 'Running: %s index %s' % (inputs.samtools, s._bamCalibNormal))
        if not inputs.dry:
            os.system('%s index %s' % (inputs.samtools, s._bamCalibNormal))
        if inputs.verbose:
            print_log(inputs, 'Running: %s index %s' % (inputs.samtools, s._bamCalibTumor))
        if not inputs.dry:
    ...
    for s in targets_data_processed:
        ...
        if inputs.exome:
            if inputs.verbose:
                if not(inputs.cbed is None):
                    print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --exome --referenceFasta %s --runDir %s --callRegions %s" % (os.path.abspath(inputs.mantabin + '/configManta.py'),
                                                                                                                                     s._bamCalibNormal,
                                                                                                                                     s._bamCalibTumor,
                                                                                                                                     inputs.reference,
                                                                                                                                     s._mantaWD,
                    ...
                    print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --exome --referenceFasta %s --runDir %s" % (os.path.abspath(inputs.mantabin + '/configManta.py'),
                                                                                                                    s._bamCalibNormal,
                                                                                                                    s._bamCalibTumor,
                                                                                                                    inputs.reference,
                                                                                                                    s._mantaWD))
            ...
            if inputs.verbose:
                if not(inputs.cbed is None):
                    print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --referenceFasta %s --runDir %s --callRegions %s" % (os.path.abspath(inputs.mantabin + '/configManta.py'),
                                                                                                                             s._bamCalibNormal,
                                                                                                                             s._bamCalibTumor,
                                                                                                                             inputs.reference,
                                                                                                                             s._mantaWD,
                    ...
                    print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --referenceFasta %s --runDir %s" % (os.path.abspath(inputs.mantabin + '/configManta.py'),
                                                                                                            s._bamCalibNormal,
                                                                                                            s._bamCalibTumor,
                                                                                                            inputs.reference,
                                                                                                            s._mantaWD))
        ...
        if inputs.verbose:
            print_log(inputs, "Running: python %s -m local -j %d" % (os.path.abspath(s._mantaWD + '/runWorkflow.py'),
                                                         inputs.threads))
        ...
        for s in targets_data_processed:
            ...
            if inputs.exome:
                if inputs.verbose:
                    if not(inputs.cbed is None):
                        print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --referenceFasta %s --indelCandidates %s --exome --callRegions %s --runDir %s" % (os.path.abspath(inputs.strelkabin + '/configureStrelkaSomaticWorkflow.py'),
                                                                                                                                                                  s._bamCalibNormal,
                                                                                                                                                                  s._bamCalibTumor,
                                                                                                                                                                  inputs.reference,
                        ...
                        print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --referenceFasta %s --indelCandidates %s --exome --runDir %s" % (os.path.abspath(inputs.strelkabin + '/configureStrelkaSomaticWorkflow.py'),
                                                                                                                                                                  s._bamCalibNormal,
                                                                                                                                                                  s._bamCalibTumor,
                                                                                                                                                                  inputs.reference,
                                                                                                                                                                  os.path.abspath(s._mantaWD + '/results/variants/candidateSmallIndels.vcf.gz'),
                ...
                if inputs.verbose:
                    if not(inputs.cbed is None):
                        print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --referenceFasta %s --indelCandidates %s --callRegions %s --runDir %s" % (os.path.abspath(inputs.strelkabin + '/configureStrelkaSomaticWorkflow.py'),
                                                                                                                                                                  s._bamCalibNormal,
                                                                                                                                                                  s._bamCalibTumor,
                                                                                                                                                                  inputs.reference,
                                                                                                                                                                  os.path.abspath(s._mantaWD + '/results/variants/candidateSmallIndels.vcf.gz'),
                        ...
                        print_log(inputs, "Running: python %s --normalBam %s --tumorBam %s --referenceFasta %s --indelCandidates %s --runDir %s" % (os.path.abspath(inputs.strelkabin + '/configureStrelkaSomaticWorkflow.py'),
                                                                                                                                                                  s._bamCalibNormal,
                                                                                                                                                                  s._bamCalibTumor,
                                                                                                                                                                  inputs.reference,
                                                                                                                                                                  os.path.abspath(s._mantaWD + '/results/variants/candidateSmallIndels.vcf.gz'),
...
def ngs_strelka_germline(inputs, targets_data_processed):
    ...
    if os.path.exists(os.path.abspath(inputs.dir + 'STRELKA_RUN_GERMLINE')):
        print_log(inputs, "    *** Strelka run directory found, skipping...")
        ...
        if inputs.exome:
            if inputs.verbose:
                if not(inputs.cbed is None):
                    print_log(inputs, "Running python %s --referenceFasta %s --exome --runDir %s --callRegions %s --bam %s" % (inputs.strelkabin + '/configureStrelkaGermlineWorkflow.py',
                                                                                                                  inputs.reference,
                                                                                                                  runDir,
                                                                                                                  inputs.cbed,
                                                                                                                  " --bam ".join(files)))
                else:
                    print_log(inputs, "Running python %s --referenceFasta %s --exome --runDir %s --bam %s" % (inputs.strelkabin + '/configureStrelkaGermlineWorkflow.py',
                                                                                                 inputs.reference,
                                                                                                 runDir,
                                                                                                 " --bam ".join(files)))
            if not inputs.dry:
            ...
            if inputs.verbose:
                if not(inputs.cbed is None):
                    print_log(inputs, "Running python %s --referenceFasta %s --runDir %s --callRegions %s --bam %s" % (inputs.strelkabin + '/configureStrelkaGermlineWorkflow.py',
                                                                                                                  inputs.reference,
                                                                                                                  runDir,
                                                                                                                  inputs.cbed,
                                                                                                                  " --bam ".join(files)))
                else:
                    print_log(inputs, "Running python %s --referenceFasta %s --runDir %s --bam %s" % (inputs.strelkabin + '/configureStrelkaGermlineWorkflow.py',
                                                                                             inputs.reference,
                                                                                             runDir,
                                                                                             " --bam ".join(files)))
            if not inputs.dry:
